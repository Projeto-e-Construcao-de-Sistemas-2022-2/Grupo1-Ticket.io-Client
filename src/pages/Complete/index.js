...
function Complete() {
  ...
  let postData = async (data) => {
    await axios.post(process.env.REACT_APP_SERVER+'/user', data)
    .then(function(res){
        ...
        >
          <form onSubmit={handleSubmit(onSubmit)}>
            <div className="rounded row d-flex align-items-center m-auto col-10 col-md-6 col-lg-5 col-xl-3 bg-primary text-light p-4 p-md-5 ">
              <p className="mt-3 mb-4 text-center fs-3 text-uppercase user-select-none">Completar cadastro</p>
              <label htmlFor="name" className="fs-6">Nome completo</label>
              <input type="text" readOnly name="name" id="name" className="form-control bg-dark text-light mb-3" value={user.displayName} {...register("name")} />
              <label htmlFor="email" className="fs-6">E-mail</label>
              <input type="email" readOnly name="email" id="email" className="form-control bg-dark text-light mb-3" value={user.email} {...register("email")} />
              <label htmlFor="cpf" className="fs-6">CPF</label>
              <input type="number" name="cpf" id="cpf" className="form-control mb-3" {...register("cpf", {
              ...
              <input type="number" name="cep" id="cep" className="form-control mb-3" {...register("cep", {
                required: "Campo obrigatório", 
                minLength: {
                  value: 8,
                  message: "8 digitos!"
              ...
              <button type="submit" className="mb-5 btn btn-outline-light">Finalizar</button>
            </div>
